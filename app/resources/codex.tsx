...
export default function CodexScreen() {
  ...
  const handleEntryPress = (entryId: string) => {
    HapticFeedback.light();
    router.push(`/resources/codex/${entryId}` as any);
  };
  ...
  return (
    <View style={styles.container}>
      <LinearGradient
        colors={[theme.colors.indigo + '20', ...theme.colors.backgroundGradient.slice(1)]}
      ...
      >
        <SafeAreaView style={styles.safeArea} edges={['top']}>
          <Animated.View style={[styles.animatedContainer, animatedStyle]}>
            ...
            >
              <ParticleEffect count={8} color={theme.colors.indigo + '30'} />
...
const EntryCard: React.FC<{ entry: any }> = ({ entry }) => {
  ...
  return (
    ...
    >
      <LinearGradient
        colors={[theme.colors.indigo + '40', theme.colors.violet + '20', theme.colors.cardBg]}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
        style={[styles.entryCard, { borderColor: theme.colors.border }]}
      >
        <Text style={[styles.entryName, { color: theme.colors.textPrimary, fontSize: 18 * textScale }]}>
          {entry.name}
        </Text>
        <Text style={[styles.entryCategory, { color: theme.colors.indigo, fontSize: 12 * textScale }]}>
          {entry.category}
        </Text>
        <Text style={[styles.entryDescription, { color: theme.colors.textSecondary, fontSize: 13 * textScale }]} numberOfLines={2}>
          {entry.description}
        </Text>
        <View style={[styles.entryBorder, { borderColor: theme.colors.indigo + '60' }]} />
      </LinearGradient>
    </Animated.View>
  );
};

...
import { getGlossaryEntryById } from '@/data/paranormal/glossary';
    ...
    >
      ...
      >
        <Animated.View style={animatedStyle}>
          <LinearGradient
            colors={[theme.colors.gold + '30', theme.colors.gold + '15', theme.colors.cardBg]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
            style={[styles.sectionCard, { borderColor: theme.colors.gold + '60' }]}
          >
            <View style={styles.sectionHeader}>
              <Text style={[styles.sectionTitle, { color: theme.colors.textPrimary, fontSize: 16 * textScale }]}>
                {section.title}
              </Text>
              <Text style={[styles.expandIcon, { color: theme.colors.gold }]}>
                {expanded ? '▼' : '▶'}
              </Text>
            </View>
            ...
            <View style={[styles.sectionCardBorder, { borderColor: theme.colors.gold + '40' }]} />
          </LinearGradient>
        </Animated.View>
      </TouchableOpacity>
    </Animated.View>
...
export default function GlossaryDetailScreen() {
  ...
  useEffect(() => {
    const loadEntry = async () => {
      ...
      const entryData = getGlossaryEntryById(id as string);
      setEntry(entryData);

      const favoriteStatus = await storage.isFavorite(`glossary-${id}`);
  ...
  const handleCopyToClipboard = () => {
    ...
    Alert.alert('Copied!', 'Term details copied to clipboard');
  };
      ...
      <View style={styles.container}>
        ...
        >
          <SafeAreaView style={styles.safeArea} edges={['top']}>
            <Text style={[styles.errorText, { color: theme.colors.textPrimary }]}>Term not found</Text>
          </SafeAreaView>
        </LinearGradient>
      </View>
    );
  ...
  return (
    <View style={styles.container}>
      <LinearGradient
        colors={[theme.colors.gold + '25', ...theme.colors.backgroundGradient.slice(1)]}
        style={styles.gradient}
        start={{ x: 0, y: 0 }}
        end={{ x: 0, y: 1 }}
      >
        <SafeAreaView style={styles.safeArea} edges={['top']}>
          <Animated.View style={[styles.animatedContainer, animatedStyle]}>
            <ParticleEffect count={12} color={theme.colors.gold + '25'} />
            ...
            <View style={styles.header}>
              ...
              <View style={styles.headerContent}>
                <Text style={[styles.termTitle, { color: theme.colors.textPrimary, fontSize: 28 * textScale }]}>
                  {entry.term}
                </Text>
                <Text style={[styles.termCategory, { color: theme.colors.gold, fontSize: 14 * textScale }]}>
                  {entry.category}
                </Text>
                <Text style={[styles.termDefinition, { color: theme.colors.textSecondary, fontSize: 14 * textScale }]}>
                  {entry.definition}
                </Text>
              </View>
            </View>
